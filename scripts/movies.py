...
def in_theaters(context, category):
    results = browser.browse(query=query(context=context,
                                         category=BrowseMovieInTheaterCategory.category(value=category)
                                                                              .value[0]["client_category"]))
...
def streaming(context, category):
    results = browser.browse(query=query(context=context,
                                         category=BrowseStreamingMovieCategory.category(value=category)
                                                                              .value[0]["client_category"]))
    click.echo(table_builder.build(movies=results))


def query(context, category):
    ...
    return MovieBrowsingQuery(minimum_rating=context.obj["MINIMUM_RATING"],
